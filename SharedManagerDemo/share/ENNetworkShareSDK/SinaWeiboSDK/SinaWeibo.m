...
+ (SinaWeibo *)sharedSinaWeibo
{
    __strong static id _sharedInstance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance = [[SinaWeibo alloc] init];
    });
...
- (void)setSinaWeiboAppKey:(NSString *)_appKey
                           ...
                           delegate:(id<SinaWeiboRequestDelegate>)_delegate
{
    ...
    if ([self isAuthValid])
    {
        ...
        //add by hudongfei
        [params setValue:self.appKey forKey:@"source"];
...
{
//    NSLog(@"self.ssoCallbackScheme :%@",self.ssoCallbackScheme);
